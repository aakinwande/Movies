...
function App() {
  ...
  const fetchMovies = async (searchKey) => {
    ...
    setMovies(results);
    await selectMovieVid(results[0]);
  };

  const fetchMovieVid = async (id) => {
  ...
  const selectMovieVid = async (movie) => {
    setPlayTrailer(false);
    const data = await fetchMovieVid(movie.id);
  ...
  const renderMovies = () =>
    movies.map((movie) => (
      <MovieCard key={movie.id} movie={movie} selectMovie={selectMovieVid} />
    ));
